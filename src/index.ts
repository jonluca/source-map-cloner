...
import { processSourceMap } from "./source-map-parser.js";
import {
  discoverJavaScriptFiles,
  getFallbackSourceMapUrl,
} from "./js-discovery.js";
import { getSourceMappingURL } from "./utils.js";
import { fetchFromURL } from "./fetch-utils.js";
...
async function fetchAndParseJsFile(
  ...
  try {
    ...
    for (const sourceMapUrl of urlsToCheck) {
      try {
        ...
        if (sourceContent && !sourceContent.startsWith("<")) {
          if (verbose) {
            logger.info(`Found source map content: ${sourceMapUrl}`);
          }
          ...
          logger.info(`No source map content for: ${sourceMapUrl}`);
        }
      } catch (error) {
        if (verbose) {
          logger.warn(
    ...
    logger.error(`Error fetching JS file ${url}: ${error}`);
    if (verbose) {
      console.error(error);
    }
  }
    ...
    for (const files of fileArrays) {
      for (const file of files) {
        ...
        if (!result.files.has(file.path)) {
          ...
          logger.warn(`Duplicate file skipped: ${file.path}`);
        }
      }
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    crawler.on("drain", async () => {
      try {
        ...
        if (options.verbose) {
          logger.info(`Finished crawling ${options.baseUrl}`);
        }
        resolve();
      } catch (error) {
        reject(error);
